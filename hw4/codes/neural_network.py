...
def get_predictions(data, W1, W2):
...
train_loss = [get_loss(y_train, get_predictions(X_train, W1, W2)) / 60000]
test_loss = [get_loss(y_test, get_predictions(X_test, W1, W2)) / 10000]
error = []

print(get_predictions(X_test[:, 0], W1, W2))
...
while it <= epochs:
    ...
    for b in range(int(batches)):
        ...
        g = get_predictions(X_batch, W1, W2)
        d2 = (g - y_batch) # * a3 * (1 - a3)
        d1 = np.matmul(W2.T, d2) * a1 * (1 - a1)
        W2 = W2 - h * np.matmul(d2, a1.T)
        W1 = W1 - h * np.matmul(d1, X_batch.T)
    print(get_predictions(X_test[:, 0], W1, W2))
    print(it, get_loss(y_train, get_predictions(X_train, W1, W2)) / 60000)
    train_loss.append(get_loss(y_train, get_predictions(X_train, W1, W2)) / 60000)
    test_loss.append(get_loss(y_test, get_predictions(X_test, W1, W2)) / 10000)
    pred = get_predictions(X_test, W1, W2)
    e = 0
...
plt.savefig("hw4/figures/learning_curve.png")
